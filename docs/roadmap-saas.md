...
This document maps the path from the current single-tenant local app to the multi-tenant platform described in `pitch.md`. Each phase has a concrete technical scope, a user-facing outcome, and an exit condition before the next phase begins.
...
*See also: `pitch.md` for the investor narrative, `app-organisation.md` for the current product structure, `deployment.md` for the current hosting setup.*
